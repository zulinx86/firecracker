...
pub(crate) mod tests {
    ...
    fn test_enable_intel_amx() {
        ...
        if current_version >= merged_version {
            let mut permitted_xfeatures: libc::c_ulong = 0;
            // SAFETY: Safe because the second input (`op`) should be valid on kernel v5.17+, and
            // the third input (`addr`) is a valid `c_ulong` pointer.
            SyscallReturnCode(unsafe {
                libc::syscall(
                    libc::SYS_arch_prctl,
