...
fn respond_to_put_request(mmds: &mut Mmds, request: Request) -> Response {
    ...
    match result {
        Ok(token) => {
            ...
            // - `header` is either `X_METADATA_TOKEN_TTL_SECONDS_HEADER` or `X_AWS_EC2_METADATA_TOKEN_SSL_SECONDS_HEADER`.
            // - `ttl_seconds` is a decimal number between `MIN_TOKEN_TTL_SECONDS` and `MAX_TOKEN_TTL_SECONDS`.
